...
from .utils import TextEncoder, TextVisualCA, ViewSpatialAttention, HyperNet, TargetNet
...
class ViTWClip(nn.Module):
    def __init__(
        ...
        self.fc_cls = nn.Linear(1024,5488)
        self.fc_quality = nn.Linear(1024,224)
    ...
    def forward(self, view_xs, d_images, order, prompt, prompt_zh):
        ...
        loss_pred = self.calculate_cos(pred_cls) - self.calculate_cos(self.pred_cls_f).detach()
        ...
        prompt_cls = self.fc_cls(torch.cat([prompt_eot, prompt_all_zh[:,0,:]], dim=-1))
        param = self.hypernet(prompt_cls.reshape(-1,112,7,7))
        ...
        scores = []
        for i in range(pred_cls.size(1)):
            pred_embedding = pred_cls[:,i,:]
            pred_embedding = self.fc_quality(pred_embedding).unsqueeze(2).unsqueeze(3)
            score = targetnet(pred_embedding)
            scores.append(score)
            
        scores = torch.stack(scores)

        return scores, loss_pred
